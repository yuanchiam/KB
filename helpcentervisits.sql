...
case when kb.language_id is null then api.language_id 
     when kb.language_id in ('es-es','fr-ca','pt-pt','zh-tw','zh-cn') then substr(lower(kb.language_id),1,2) else kb.language_id end as language_tmp,
...
case when lower(node.language_id) in ('es-es','fr-ca','pt-pt','zh-tw','zh-cn') then substr(lower(node.language_id),1,2)
else lower(node.language_id) end as language_id
...
-- populate language_id if english or "error" articles 
...
and application_id='padme' group by 1,2,3,4) kb

on kb.node_id=api.node_id and kb.application_id=api.application_id
group by 1,2,3,4,5) kb1

left join

(select language_iso_code, language_desc
from dse.dea_language_d) lang

on kb1.language_tmp=lang.language_iso_code
